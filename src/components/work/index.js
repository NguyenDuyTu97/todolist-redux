...
function Work(props) {
  const { users, handleChangeStatusUser, handleDeleteUser, handleEditUser } =
    props;
  ...
  function onDragEnd(result) {
    ...
    if (sInd === dInd) {
      ...
      const itemDropSource = reorder(
        newUsers[indexDropSource],
        source.index,
        destination.index
      );
        ...
        <DragDropContext onDragEnd={onDragEnd}>
          {usersUpdate.map((el, ind) => (
            <div className="text-center">
              ...
              <Droppable key={el.status} droppableId={`${el.status}`}>
                {(provided, snapshot) => (
                  <div
